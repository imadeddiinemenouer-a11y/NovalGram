    ...
    return (
      <div className={`min-h-screen flex items-center justify-center ${isDark ? 'bg-gray-950' : 'bg-gray-50'}`}>
        <div className="text-center">
          ...
          <h2 className={`text-xl font-semibold mb-2 ${isDark ? 'text-white' : 'text-gray-900'}`}>Sign in to view notifications</h2>
          <button
      ...
      <div className={`sticky top-14 z-30 shadow-sm border-b ${isDark ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-200'}`}>
        <div className="max-w-4xl mx-auto px-4 py-4">
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Bell className={`w-6 h-6 ${isDark ? 'text-red-400' : 'text-indigo-600'}`} />
            Notifications
          </h1>
        </div>
      </div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            {notifications.map((notif) => {
              ...
              const color = colorMap[notif.type] || { light: 'bg-gray-100 text-gray-600', dark: 'bg-gray-800 text-gray-400' };
              return (
                <div
                  key={notif.id}
                  ...
                  } ${!notif.is_read ? `ring-2 ${isDark ? 'ring-red-500/50' : 'ring-indigo-300'}` : ''}`}
                >
                  <div className="flex items-start gap-3">
                    ...
                    {!notif.is_read && (
                      <span className={`w-2.5 h-2.5 rounded-full flex-shrink-0 mt-2 ${
                        isDark ? 'bg-red-500' : 'bg-indigo-500'
                      }`} />
                    )}
                  </div>
                </div>
