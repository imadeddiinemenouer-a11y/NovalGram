...
export default function BottomNav() {
  ...
  return (
    <nav className="flex bg-[var(--ink)] border-t border-[var(--border)] flex-shrink-0 z-50" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
      {items.map((item) => (
        <button
          key={item.path}
          ...
          className={`flex-1 flex flex-col items-center gap-1 py-2.5 border-none bg-transparent cursor-pointer transition-colors ${
            isActive(item.path) ? 'text-[#f0eff8]' : 'text-[var(--muted)]'
          }`}
        >
      ...
      >
        <div className="w-[46px] h-[28px] rounded-full -mt-3.5 bg-[#f0eff8] flex items-center justify-center">
          <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#0f0f14" strokeWidth="2.5">
            <line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" />
          </svg>
        </div>
        <span className="text-[10px] font-semibold text-[var(--muted)] uppercase tracking-wider mt-0.5">Write</span>
      </button>
    </nav>
  );
}